...
static
FRESULT dir_sdi (
	FATFSDIR *dj,		/* Pointer to directory object */
...
static
FRESULT dir_find (
	FATFSDIR *dj			/* Pointer to the directory object linked to the file name */
)
{
	FRESULT res;
	BYTE c, *dir;
...
static
FRESULT dir_read (
	FATFSDIR *dj			/* Pointer to the directory object that pointing the entry to be read */
)
{
	FRESULT res;
	BYTE c, *dir;
...
FRESULT dir_register (	/* FR_OK:Successful, FR_DENIED:No free entry or too many SFN collision, FR_DISK_ERR:Disk error */
	FATFSDIR *dj				/* Target directory with object name to be created */
)
{
	FRESULT res;
	BYTE c, *dir;
...
FRESULT dir_remove (	/* FR_OK: Successful, FR_DISK_ERR: A disk error */
	FATFSDIR *dj				/* Directory object pointing the entry to be removed */
)
{
	FRESULT res;
#if _USE_LFN	/* LFN configuration */
...
static
FRESULT create_name (
	FATFSDIR *dj,			/* Pointer to the directory object */
...
FRESULT follow_path (	/* FR_OK(0): successful, !=0: error code */
	FATFSDIR *dj,			/* Directory object to return last directory and found object */
...
//	if (1) {			/* Nul path means the start directory itself */	
		res = dir_sdi(dj, 0);
		dj->dir = 0;
...
)
{
	if (!fs)/*{ ||  || )*/{
		...
		return (FR_INVALID_OBJECT + 0x01);
	}else if(!fs->fs_type){
		return (FR_INVALID_OBJECT + 0x02);
	}else if(fs->id != id){
		return (FR_INVALID_OBJECT + 0x03);
	}
...
FRESULT f_opendir (
	FATFSDIR *dj,			/* Pointer to directory object to create */
	const TCHAR *path	/* Pointer to the directory path */
)
{
	FRESULT res;
...
FRESULT f_readdir (
	FATFSDIR *dj,			/* Pointer to the open directory object */
		...
		if (res == FR_OK) {					/* The object is accessible */
			...
			if (res == FR_OK && (dir[DIR_Attr] & AM_DIR)) {	/* Is it a sub-dir? */
				if (dclst < 2) {
					...
					mem_cpy(&sdj, &dj, sizeof(FATFSDIR));	/* Check if the sub-dir is empty or not */
					sdj.sclust = dclst;
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {						/* Old object is found */
			if (!djo.dir) {						/* Is root dir? */
				...
				mem_cpy(&djn, &djo, sizeof(FATFSDIR));		/* Check new object */
